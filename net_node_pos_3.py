...
#             return False#  ### FALSE 값을 리턴!!!!
    ...
    def is_chain_valid(self, chain): #연결된 노드들의 적정성 확인
...
mine_owner = 'validator_3'
mine_profit = 0.1

blockchain = Blockchain(account_name=mine_owner, account_weight= 2) #2개 예치 전체 
...
def mine():
    ...
    #####
    ################# 노드 연결을 위해 추가되는 부분
    for node in blockchain.nodes: # nodes에 연결된 모든 노드에 PoS가 완료되었음을 전파
        headers = {'Content-Type' : 'application/json; charset=utf-8'}
...
def register_nodes():
    ...
    if registering_node.split("//")[1] in blockchain.nodes:
        print("Node already registered")  # 이미 등록된 노드.
        response = {
...
@app.route('/nodes/node_weight', methods=['GET']) # PoS를 위해 각 노드마다 staking 토큰량 확인
def node_weight():
